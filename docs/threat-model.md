...
(SLSA provenance, VEX, VSA, Notation signatures, SBOMs) from OCI registries.
...
| **Denial of service** | TUF mirror or Sigstore services are unavailable                      | Stale trusted root cache (up to 24h) allows continued verification; metric `trusted_root_stale_total` tracks usage; startup pre-warms the cache          |
...
| Stale trusted root                    | If the TUF mirror is unreachable, the plugin uses a cached trusted root for up to 24 hours. Revoked certificates or keys remain trusted during this window.                                                                                          | Medium   | Monitor `trusted_root_stale_total` metric; alert on prolonged staleness                       |
