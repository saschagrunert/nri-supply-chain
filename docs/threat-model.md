...
| **Denial of service** | Oversized or excessive attestations exhaust plugin memory                                  | Per-attestation size limit (`maxAttestationSize`), aggregate limit (`maxTotalAttestationSize`), max referrers (`maxReferrers`) in `internal/attestation/fetcher_collect.go`; global fetch concurrency semaphore (50) and per-host semaphore (10) |
...
| **Info disclosure**   | Key files or config readable by unprivileged users                    | Key files validated via `Lstat` (detects symlinks); `fileutil.ReadLimited` caps reads; `fileutil.CheckCredentialPermissions` warns on files with mode bits more permissive than 0600; standard Unix permissions apply              |
...
| Category              | Threat                                                                                    | Mitigation                                                                                                                                                                                                                      |
| --------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Spoofing**          | Attacker places a crafted bundle in the store directory                                   | Bundle manifest signatures (ECDSA, Ed25519, RSA) verify authenticity; `require_bundle_signature` rejects unsigned bundles; `bundle_signature_key` path must not be a symlink                                                    |
| **Tampering**         | Attacker modifies attestation blobs after import while keeping the signed manifest intact | `Import()` verifies blob integrity (SHA-256 digest and size) after signature verification and before committing to the store; `bundle verify` CLI also checks blob integrity                                                    |
...
| Bundle store modification after import  | An attacker with write access to the bundle store directory can modify attestation blobs after import. Integrity is verified during import but not on every read (performance trade-off). The signed manifest prevents undetected changes to image mappings, but individual blob contents could be altered. | Low      | Restrict filesystem permissions on `attestation_store`; re-run `bundle verify` after suspicious changes; use `require_bundle_signature` to detect manifest tampering |
